# Panduan Pengguna - Sistem Manajemen Servis UMKM

Selamat datang di Sistem Manajemen Servis UMKM. Panduan ini akan membantu Anda memahami dan menggunakan fitur-fitur aplikasi kami secara maksimal untuk meningkatkan efisiensi dan transparansi bisnis Anda.

## Daftar Isi
1.  [Tentang Aplikasi](#tentang-aplikasi)
2.  [Memulai](#memulai)
3.  [Fitur Utama: Pencatatan Servis](#fitur-utama-pencatatan-servis)
    *   [Langkah 1: Informasi Pelanggan dan Keluhan](#langkah-1-informasi-pelanggan-dan-keluhan)
    *   [Langkah 2: Diagnosis Kerusakan dan Rencana Perbaikan](#langkah-2-diagnosis-kerusakan-dan-rencana-perbaikan)
    *   [Langkah 3: Tinjau dan Simpan Laporan](#langkah-3-tinjau-dan-simpan-laporan)
4.  [Fitur Tambahan](#fitur-tambahan)
    *   [Manfaat Akun Pengguna](#manfaat-akun-pengguna)
    *   [Kustomisasi Kop Surat](#kustomisasi-kop-surat)
    *   [Riwayat Servis](#riwayat-servis)

---

## Tentang Aplikasi

Aplikasi ini dirancang untuk membantu para pelaku UMKM dalam mencatat setiap proses servis secara transparan, mulai dari penerimaan barang hingga selesai perbaikan. Dengan pencatatan yang detail, Anda dapat meningkatkan kepercayaan pelanggan dan profesionalisme bisnis Anda.

## Memulai

Untuk memulai, ikuti langkah-langkah berikut:
1.  Tekan tombol **Mulai** pada halaman utama.
2.  Sebuah jendela *pop-up* akan muncul. Anda dapat memilih untuk **Masuk (Login)** atau **Daftar (Register)** untuk mengakses fitur tambahan, atau melanjutkan tanpa akun.
3.  Anda akan diarahkan ke halaman aplikasi utama untuk memulai proses pencatatan servis.

## Fitur Utama: Pencatatan Servis

Proses pencatatan servis dibagi menjadi tiga langkah mudah:

### Langkah 1: Informasi Pelanggan dan Keluhan
Pada langkah ini, Anda akan diminta untuk memasukkan:
*   **Informasi Pelanggan:** Nama dan kontak (nomor telepon atau email).
*   **Informasi Perangkat:** Nama perangkat (misalnya, laptop, ponsel).
*   **Keluhan:** Deskripsi keluhan yang disampaikan oleh pelanggan.

### Langkah 2: Diagnosis Kerusakan dan Rencana Perbaikan
Setelah memeriksa perangkat, catat temuan Anda secara detail:
*   **Kerusakan:** Catat setiap kerusakan yang ditemukan dan tentukan tingkat keparahannya.
*   **Rencana Perbaikan:** Tuliskan semua tindakan perbaikan yang diperlukan, termasuk:
    *   **Jenis:** Apakah perbaikan berupa barang (penggantian suku cadang) atau jasa.
    *   **Harga:** Masukkan harga untuk setiap item perbaikan.
*   **Status Servis:** Perbarui status pengerjaan servis (misalnya, "Dalam Pengerjaan", "Menunggu Suku Cadang").
*   **Total Harga:** Akan dihitung secara otomatis berdasarkan daftar perbaikan.

Setelah semua informasi terisi, klik tombol **Selesai**.

### Langkah 3: Tinjau dan Simpan Laporan
Di langkah terakhir, Anda dapat:
*   **Melihat Pratinjau:** Tinjau kembali seluruh informasi dalam format laporan yang siap dibagikan.
*   **Simpan dan Unduh:** Simpan laporan dalam format digital untuk diunduh. Laporan ini dapat Anda bagikan kepada pelanggan sebagai bukti servis yang transparan. Jika Anda masuk ke akun Anda, laporan ini juga akan tersimpan secara otomatis di riwayat servis Anda.

## Fitur Tambahan

### Manfaat Akun Pengguna
Meskipun aplikasi dapat digunakan tanpa akun, membuat akun memberikan Anda beberapa keuntungan:
*   **Riwayat Servis:** Semua catatan servis akan tersimpan dengan aman dan dapat diakses kapan saja di halaman **Riwayat**.
*   **Kustomisasi Kop Surat:** Tambahkan identitas bisnis Anda pada setiap laporan servis.

### Kustomisasi Kop Surat
Anda dapat menambahkan kop surat (logo dan nama bisnis) pada laporan Anda. Informasi ini dapat diisi saat pertama kali mendaftar dan dapat diubah kapan saja melalui halaman **Profil**.

### Riwayat Servis
Akses semua riwayat transaksi servis Anda di halaman **Riwayat**. Anda dapat melihat detail setiap servis dan mencetak ulang laporan jika diperlukan.

Terima kasih telah menggunakan Sistem Manajemen Servis UMKM!
